...
def stats_term_class_ndf(doc_term_list, doc_class_list, term_dict, class_dict): #计算每个词在对应类别的文档中出现的次数（不是文档数）
...
def stats_term_class_df(doc_term_list, doc_class_list, term_dict, class_dict): #计算每个词在对应类别的文档中出现的文档频数
    term_class_df_mat = np.zeros((len(term_dict), len(class_dict)), np.float64)
    for k in range(len(doc_class_list)):
        term_flag = {}.fromkeys(term_dict.keys(), 0)
        class_index = class_dict[doc_class_list[k]]
        doc_terms = doc_term_list[k]
        for term in doc_terms.split():
            term_flag[term] += 1
...
def feature_selection_ig(class_df_list, term_set, term_class_df_mat):
    ...
    N = sum(sum(term_class_df_mat)) #所有词出现的总数
...
def feature_selection(doc_term_list, doc_class_list, fs_method):
    ...
    term_class_df_mat = stats_term_class_df(doc_term_list, doc_class_list, term_dict, class_dict)  # 字典对应的词在不同类别下的文档频率
    term_class_ndf_mat = stats_term_class_ndf(doc_term_list, doc_class_list, term_dict, class_dict)  # 字典对应的词在不同类别下的总数
    ...
    if fs_method == 'IG':
        print 'IG'
        term_set_fs = feature_selection_ig(class_df_list, term_set, term_class_ndf_mat)
        ...
        print 'MI'
        term_set_fs = feature_selection_mi(class_df_list, term_set, term_class_ndf_mat)
