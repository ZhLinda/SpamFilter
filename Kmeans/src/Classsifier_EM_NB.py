...
if __name__ == "__main__":
    ...
    set_part(ori_data_filename, ori_class_filename, Dl_data_filename, Dl_class_filename, Du_data_filename,
             Du_class_filename)
             


    Cl = load_class_data('../data/derived/Dl_class.txt')
    Dl = load_train_data('../data/derived/Dl_data.txt')
    Cu = load_class_data('../data/derived/Du_class.txt')
    Du = load_train_data('../data/derived/Du_data.txt')
    #Duu = load_train_data("../data/derived/train_allwords.txt")
    #Cuu = load_class_data("../data/derived/classLabel.txt")
    ...
    test_vector = word2vec_setOfWords(Du, dictionary)
    classify_result = []
    ...
    '''
    test_vector_2 = word2vec_tfidf(Duu, dictionary)
    classify_result_2 = []
    for vec2classify in test_vector_2:
        label, t1, t2 = classify(vec2classify, p_w_ham, p_w_spam, p_spam)
        classify_result_2.append(label)
    evaluate(classify_result_2, Cuu)
    '''



    fp = open("../data/derived/EM_results.txt","w")
    for item in classify_result:
        fp.write(item.__str__() + "\n")