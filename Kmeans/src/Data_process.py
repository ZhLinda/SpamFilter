...
def data_process():
    ...
    print "----------正在进行文本分词----------"
    get_data_segmented(stopWords,"../data/all.txt")  # 使用JIEBA分词对所有文本进行分词
    del stopWords
    gc.collect()
    print "------------文本分词完成------------\n"

    print "----------正在进行文本bigram切分----------"
    countNgram(2,'../data/all.txt','../data/derived/train_ngram.txt')
    print "------------文本bigram切分完成------------\n"
    ...
    #dictionary = load_dictionary("../model/new_new_dict.txt")
