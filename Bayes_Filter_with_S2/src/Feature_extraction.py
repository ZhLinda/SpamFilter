...
def stats_term_df(doc_terms_list,term_dict): #计算每个词的文档频率
...
def stats_term_class_df(doc_term_list, doc_class_list, term_dict, class_dict): #计算每个词在对应类别的文档中出现的次数（不是文档数）
...
def stats_term_class_ndf(doc_term_list, doc_class_list, term_dict, class_dict): #计算每个词在对应类别的文档中出现的文档频数
    term_class_df_mat = np.zeros((len(term_dict), len(class_dict)), np.float64)
    for k in range(len(doc_class_list)):
