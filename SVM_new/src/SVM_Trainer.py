...
class TrainerLinear:
    def __init__(self, training_data, training_target, test_data, test_target, names):
    ...
    def train_classifier(self):
        ...
        print "模型训练完毕，已成功保存！\n"
        print self.clf.coef_  # weights signed to the features

        '''
        print "----------正在进行最优特征选择----------"
        rfecv = RFECV(estimator=self.clf,step=1,cv=StratifiedKFold(training_target,2),scoring='f1_macro')
        rfecv.fit(training_data,training_target)
        print "最优的特征数量为: %d " % rfecv.n_features_
        print rfecv.ranking_
        print "\nFeatures sorted by their rank:"
        result = sorted(zip(map(lambda x: round(x, 4), rfecv.ranking_), names))
        print result
        fp = open("result.txt", "w")
        fp2 = open("../model/new_dict.txt","w")
        for item in result:
            fp.write(item[0].__str__() + " " + item[1].encode("utf-8") + "\n")
            fp2.write(item[1].encode("utf-8") + "\n")
        '''
...
if '__main__' == __name__:
    ...
    content = load_train_vectors(5000, 29,"../model/train_vectors.txt")
    ...
    training_data, test_data, training_target, test_target = train_test_split(content, label, test_size=0.9999999999999999) #划分训练集和测试集
    print "----------正在进行特征向量降维----------"
    #training_data, test_data = dimensionality_reduction(training_data, test_data, type='pca')
    count_time(time0)


    Trainer = TrainerLinear(training_data, training_target, test_data, test_target,names)
