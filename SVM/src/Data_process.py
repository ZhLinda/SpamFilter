...
if __name__ == "__main__":
    ...
    countNgram(2,'../data/fixed/train_data_5000.txt','../data/derived/train_ngram_5000.txt')
    print "------------文本bigram切分完成------------\n"

    print "----------正在合并分词结果----------"
    merge_words("../data/derived/train_seg_5000.txt","../data/derived/train_ngram_5000.txt","../data/derived/train_allwords_5000.txt")
    print "----------合并分词结果完成----------\n"


    print "----------正在加载类别信息----------"
    #class_label = load_class_data('../data/derived/classLabel_5000.txt')
    train_label = load_class_data('../data/derived/classLabel_5000.txt')
    print "----------类别信息加载完成----------\n"

    print "----------正在加载语料库----------"
    #train_corpus = load_train_data('../data/derived/train_allwords_5000.txt')
    train_data = load_train_data('../data/derived/train_allwords_5000.txt')
    print "----------语料库加载完成----------\n"
    ...
    del fea_1,fea_2,fea_3,fea_4,fea_5,fea_6,fea_7
    gc.collect()
    print "----------正在特征矩阵构建完成----------\n"
    ...
    save_train_vectors(train_vectors_scaled, "../model/train_vectors_5000.txt")
